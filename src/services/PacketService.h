...
public:
     ...
     * @return DataPacket*
     */
    static DataPacket* createDataPacket(uint16_t dst, uint16_t src, uint8_t type, uint8_t* payload, uint8_t payloadSize, uint8_t maxHops);
    ...
    /**
     * @brief Given a type returns if is a Carry packet
     *
     * @param type type of the packet
     * @return true True if needed
     * @return false If not
     */
    static bool isCarryPacket(uint8_t type);
