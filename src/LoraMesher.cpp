...
void LoraMesher::begin(LoraMesherConfig config) {
    ...
    ESP_LOGI(LM_TAG, "Initializing LoraMesher v%s", LM_VERSION);
...
void LoraMesher::initializeLoRa() {
    ...
    ESP_LOGI(LM_TAG, "Initializing radio");
...
void LoraMesher::initializeSchedulers() {
    ESP_LOGI(LM_TAG, "Setting up Schedulers");
    int res = xTaskCreate(
...
void LoraMesher::receivingRoutine() {
    ESP_LOGI(LM_TAG, "Receiving routine started");
    vTaskSuspend(NULL);
...
void LoraMesher::waitBeforeSend(uint8_t repeatedDetectPreambles) {
    ...
    ESP_LOGI(LM_TAG, "RandomDelay %d ms", (int) randomDelay);
    ...
    if (hasReceivedMessage) {
        startReceiving();
        ESP_LOGI(LM_TAG, "Preamble detected while waiting %d", repeatedDetectPreambles);
...
void LoraMesher::sendPackets() {
    ESP_LOGI(LM_TAG, "Send routine started");
    vTaskSuspend(NULL);
    ...
    for (;;) {
        ...
        while (ToSendPackets->getLength() > 0) {
            ...
            if (tx) {
                ...
                ESP_LOGI(LM_TAG, "TimeOnAir %d ms, next message in %d ms", (int) timeOnAir, (int) delayBetweenSend);
...
void LoraMesher::sendHelloPacket() {
    ESP_LOGI(LM_TAG, "Send Hello Packet routine started");

    vTaskSuspend(NULL);

    size_t maxNodesPerPacket = (PacketFactory::getMaxPacketSize() - sizeof(RoutePacket)) / sizeof(NetworkNode);

    ESP_LOGI(LM_TAG, "Max routing nodes per packet: %d", maxNodesPerPacket);
...
void LoraMesher::processPackets() {
    ESP_LOGI(LM_TAG, "Process routine started");
    vTaskSuspend(NULL);
        ...
        while (ReceivedPackets->getLength() > 0) {
            ...
            if (rx) {
                ...
                if (PacketService::isHelloPacket(type)) {
                    ...
                    ESP_LOGI(LM_TAG, "Packet not identified, deleting it");
                    incReceivedNotForMe();
                    PacketQueueService::deleteQueuePacketAndPacket(rx);
                }
            }
...
void LoraMesher::routingTableManager() {
    ESP_LOGI(LM_TAG, "Routing Table Manager routine started");
    vTaskSuspend(NULL);
...
void LoraMesher::queueManager() {
    ESP_LOGI(LM_TAG, "Queue Manager routine started");
    vTaskSuspend(NULL);
...
void LoraMesher::printHeaderPacket(Packet<uint8_t>* p, String title) {
    ...
    ESP_LOGI(LM_TAG, "Packet %s -- Size: %d Src: %X Fwd: %X Dst: %X Id: %d Type: %d Via: %X Seq_Id: %d Num: %d",
        title.c_str(),
        p->packetSize,
        p->src,
        p->fwd,
...
void LoraMesher::sendReliablePacket(uint16_t dst, uint8_t* payload, uint32_t payloadSize, uint16_t carry_to) {
    ...
    if (dst == BROADCAST_ADDR) {
        ...
        if (numOfNodes > 0) {
            ...
            for (size_t i = 0; i < numOfNodes; i++) {
                ...
                sendReliablePacket(node->address, payload, payloadSize, carry_to);
            }
            delete[] nodes;
        }
        return;
    ...
    // Put data in the initial packet (sync or carry)
    ////////////////////////////////////////////////////////////////////
    ...
    // Add the configuration packet
    packetList->Append(getStartSequencePacketQueue(dst, seq_id, numOfPackets, carry_to, payloadToSend, payloadSizeToSend));
    ////////////////////////////////////////////////////////////////////
...
void LoraMesher::processDataPacket(QueuePacket<DataPacket>* pq) {
    ...
    if (packetHistory.wasSeen(packet)) {
        ...
        ESP_LOGI(LM_TAG, "Data packet from %X for me", packet->src);
        incDataPacketForMe();
...
void LoraMesher::processDataPacketForMe(QueuePacket<DataPacket>* pq) {
    ...
    if (PacketService::isOnlyDataPacket(p->type)) {
        ...
        ESP_LOGI(LM_TAG, "ACK Packet received");
    ...
    else if (PacketService::isCarryPacket(p->type) && hasRole(ROLE_CARRIER)) {
        ESP_LOGI(LM_TAG, "Carry packet received");
        if (processSyncPacket(reinterpret_cast<QueuePacket<ControlPacket>*>(pq))) {
        ...
        processSyncPacket(reinterpret_cast<QueuePacket<ControlPacket>*>(pq));
        deleteQueuePacket = false;

        needAck = false;
    }
    else if (PacketService::isXLPacket(p->type)) {
        ESP_LOGI(LM_TAG, "Large payload Packet received");
    ...
    if (needAck) {
        ...
        ESP_LOGI(LM_TAG, "Previous packet need an ACK");
...
void LoraMesher::recalculateMaxTimeOnAir() {
    ...
    ESP_LOGI(LM_TAG, "Max Time on Air changed %d ms", (int) maxTimeOnAir);
}
...
void LoraMesher::addToSendOrderedAndNotify(QueuePacket<Packet<uint8_t>>* qp) {
    ...
    ESP_LOGV(LM_TAG, "Added packet to Q_SP, notifying sender task");
...
QueuePacket<ControlPacket>* LoraMesher::getStartSequencePacketQueue(uint16_t destination, uint8_t seq_id, uint16_t num_packets, uint16_t carry_to, uint8_t* payload, uint8_t payloadSize) {
...
void LoraMesher::addAck(uint16_t source, uint8_t seq_id, uint16_t seq_num) {
    ...
    ESP_LOGI(LM_TAG, "Sending next packet after receiving an ACK");
...
bool LoraMesher::processLargePayloadPacket(QueuePacket<ControlPacket>* pq) {
    ...
    if (configList->config->lastAck == configList->config->number) {
        joinPacketsAndForward(configList);
    }

    return true;
}

void LoraMesher::joinPacketsAndForward(listConfiguration* listConfig) {
    ...
    // Take note if we are carrying and to whom
    uint16_t carry_to = listConfig->config->carry_to;

    //TODO: When finished, clear everything? Or maintain the config until timeout?
    findAndClearLinkedList(q_WRP, listConfig);

    if (carry_to) {
        sendReliable(carry_to, p->payload, p->payloadSize);
    } else {
...
bool LoraMesher::processSyncPacket(QueuePacket<ControlPacket>* pq) {
    ...
    // We have the number of packets in the sequence, now set our number to zero for storage
    pq->packet->number = 0;
    ...
    if (listConfig == nullptr) {
        ...
        if (seq_num == 0) { // We are a single reliable packet
            joinPacketsAndForward(listConfig);
        }
        return true;
    }
...
void LoraMesher::actualizeRTT(sequencePacketConfig* config) {
    if (config->calculatingRTT == 0) {
        ...
        ESP_LOGI(LM_TAG, "Starting to calculate RTT seq_Id: %d Src: %X",
    ...
    if (node == nullptr) {
        ESP_LOGW(LM_TAG, "Node not found in the routing table");
        ESP_LOGI(LM_TAG, "Updating RTT (%u ms), seq_Id: %d Src: %X", (unsigned int) actualRTT, config->seq_id, config->source);
        return;
    }
    ...
    ESP_LOGI(LM_TAG, "Updating RTT (%u ms), SRTT (%u), RTTVAR (%u) seq_Id: %d Src: %X",
...
void LoraMesher::clearLinkedList(listConfiguration* listConfig) {
    ...
    ESP_LOGI(LM_TAG, "List size: %d", listSize);
...
void LoraMesher::managerTimeouts(LM_LinkedList<listConfiguration>* queue, QueueType type) {
    ...
    ESP_LOGI(LM_TAG, "Checking %s timeouts. Open connections %d", queueName.c_str(), queue->getLength());
...
void LoraMesher::addTimeout(sequencePacketConfig* configPacket) {
    ...
    ESP_LOGI(LM_TAG, "Timeout set to %u s", (unsigned int) (timeout / 1000));
}
...
void LoraMesher::recalculateTimeoutAfterTimeout(sequencePacketConfig* configPacket) {
    ...
    ESP_LOGI(LM_TAG, "Timeout recalculated to %u s", (unsigned int) (timeout / 1000));
}
